...
export async function placeInRoom(
  ...
  const sectionalClause = isSofa
    ...
    : "";
  // When placing a NON-sofa item (e.g. a table), any sofa/couch/bed in the room is NOT the
  // piece being replaced — the model tends to grab the biggest clear wall and erase the sofa
  // sitting there. Force it to keep the sofa and put the new piece on open floor in front.
  const keepSofaClause = !isSofa
    ? ` IMPORTANT — the room may already contain a sofa, couch, daybed or bed: it is NOT being replaced and MUST stay exactly where it is, fully visible and unchanged in the result. Place the ${productLabel} on open floor IN FRONT OF it — never on top of it, never against the wall behind it, and NEVER erase, shrink, hide or move the sofa/bed to make room. (You may move or remove small chairs only if the ${productLabel} would directly overlap them.)`
    : "";
